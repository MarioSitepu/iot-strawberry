...
export function ChartSection({ data }: ChartSectionProps) {
  ...
  const renderChart = (content: React.ReactNode) => {
    if (!mounted) {
      return <div className="h-full w-full flex items-center justify-center bg-muted/5 rounded-lg border border-dashed border-border/20" />;
    }
    return content;
  };
            ...
            {renderChart(
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={data}>
                  ...
                  <YAxis tick={{ fontSize: 12 }} tickLine={false} axisLine={false} tickFormatter={(val: number) => val.toFixed(1)} />
                  <Tooltip 
                    contentStyle={{ borderRadius: '12px', border: '1px solid rgba(0,0,0,0.1)', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                    formatter={(value: any) => [Number(value).toFixed(1), "PPM"]}
                  />
